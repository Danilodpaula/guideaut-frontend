...
export function useActivityLogs(limit: number = 10) {
  ...
  const getActivityDescription = (
    ...
    const descriptions: Record<string, { pt: string; en: string }> = {
      ...
      CREATE_VOTE: { pt: "Votou em uma recomendação", en: "Voted on a recommendation" },
      UPDATE_VOTE: { pt: "Alterou seu voto", en: "Changed vote" },
      CREATE_FAVORITE: { pt: "Favoritou uma recomendação", en: "Favorited a recommendation" },
      DELETE_FAVORITE: { pt: "Removeu dos favoritos", en: "Removed from favorites" },
